...
export default {
  ...
  async fetch ({ store, params }) {
    await store.commit('page_title', 'Data: Ease of Doing Business')
  },
  methods: {
    sumWeights() {
    ...
    async save() {
      if (this.weight_total === 100) {
        await this.$store.commit('inputs/ease_weights', [...this.weights])
      }
    },
    reset() {
    ...
    next() {
      this.$router.replace({ path: '/data/economic_risk' })
    }
  },
  data () {
    return {
      weights: [...this.$store.state.inputs.ease_weights],
      weight_total: 100,
      weight_color: "grey",
      fab: false,
      headers: [
