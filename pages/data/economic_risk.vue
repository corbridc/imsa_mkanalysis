...
export default {
  ...
  async fetch ({ store, params }) {
    await store.commit('page_title', 'Data: Economic Risk')
  },
  methods: {
    sumWeights() {
    ...
    async save() {
      if (this.weight_total === 100) {
        await this.$store.commit('inputs/econ_weights', [...this.weights])
      }
    },
    reset() {
    ...
    next() {
      this.$router.replace({ path: '/data/political_risk' })
    }
  },
  data () {
    return {
      weights: [...this.$store.state.inputs.econ_weights],
      weight_total: 100,
      weight_color: "grey",
      fab: false,
      headers: [
