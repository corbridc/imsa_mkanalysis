...
export default {
  ...
  async fetch ({ store, params }) {
    await store.commit('page_title', 'Data: Market Potential')
  },
  methods: {
    sumWeights() {
    ...
    async save() {
      this.sumWeights()
      if (this.weight_total === 100) {
        await this.$store.commit('inputs/pntl_weights', [...this.weights])
      }
    },
    reset() {
    ...
    next() {
      this.$router.replace({ path: '/data/business_ease' })
    }
  },
  data () {
    return {
      weights: [...this.$store.state.inputs.pntl_weights],
      weight_total: 100,
      weight_color: "grey",
      fab: false,
      headers: [
